...
func (hash *Set) AddItem(w http.ResponseWriter, r *http.Request) {
	...
	head := hash.buckets[item%1777]
	if head == nil {
		hash.buckets[item%1777] = &LinkedList{item, nil}
	} else {
		hash.buckets[item%1777] = &LinkedList{item, head}
	}
...
func (hash *Set) RemoveItem(w http.ResponseWriter, r *http.Request) {
	...
	if head.item == item {
		hash.buckets[item%1777] = head.next
